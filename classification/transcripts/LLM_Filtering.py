"""Round-based, query-agnostic RAG classification for transcript chunks.

Mirrors classification/sec/LLM_Filtering.py almost exactly -- same
round-based RAG design, same batch/caching mechanics, same schema. The
only real difference is scale/shape: ~896 transcripts (464 earnings_call +
432 other), ~33 chunks/doc average -- much smaller than a 10-K's ~268, so
MAX_CHUNKS_PER_DOC will rarely bind here, and the "document fully
exhausted in one round" case (get_round_candidates simply returns nothing
further for it -- no special handling needed) will be common, unlike SEC
filings where it basically never happens.
...
a decision made BETWEEN rounds, not automated inside this module:
  ...
  - Every chunk_classifications row is stamped with the round number it
    ...
    another round. There is deliberately no automated stopping rule --
    you decide, informed by the data (see classification/sec's module
    docstring for why an earlier consecutive-miss-streak heuristic was
...
company_relevant is NOT scored here (unlike the news module) -- a
transcript chunk is inherently about the company whose call/event it is,
so there's no "coincidental keyword match" problem the way an HN title
search has. It's hardcoded to true whenever a score is written (see
WRITE_SCORE_SQL), not asked of the model -- the structured tool response
only asks for ai_associated.

NOT YET RUN -- requires ANTHROPIC_API_KEY in .env. classification_prompt.md
in this directory was copied from classification/sec/ with light framing
tweaks (transcript instead of filing language) -- its worked examples are
still SEC-flavored placeholders and should be replaced with real
transcript-derived examples before trusting results, same as the SEC
prompt was refined after a manual sample-and-review pass.
"""
...
# distance) and sort. PLACEHOLDERS -- carried over from classification/sec
# unchanged; tune wording for transcript language once real results come
# back (see classification_prompt.md's placeholder-examples note).
QUERIES = [
...
# document -- see module docstring. Raised from 50 to 200 to match
# classification/sec's fix, then to 10000 after discovering the longest
# transcript actually has 204 chunks -- narrowly exceeding 200, which
# would have reproduced the exact same premature-cutoff bug SEC hit, just
# on a single document instead of nearly every high-signal filing.
...
def _tool_schema(n: int) -> dict:
    ...
    since a transcript chunk is inherently about the company whose
    call/event it is."""
    return {
...
# ---------------------------------------------------------------------------
# Scoping helper -- e.g. get_transcripts() for everything, or
# get_transcripts("earnings_call") to scope to just earnings calls. Returns
# content_ids; every other function in this module takes a content_ids
# scope so it composes with whatever scope the caller wants.
# ---------------------------------------------------------------------------

GET_TRANSCRIPTS_SQL = """
    select content_id
    from content
    where type = 'transcripts' and (%(sub_type)s::text is null or sub_type = %(sub_type)s)
"""


def get_transcripts(sub_type: str | None = None) -> list[str]:
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(GET_TRANSCRIPTS_SQL, {"sub_type": sub_type})
...
# classification/news and classification/sec, reusing chunk_batch_requests.
# ---------------------------------------------------------------------------
...
def _build_request(
    ...
    # name, then the chunk text (a spoken-transcript excerpt here, not a
    # single title).
    lines = []
    for i, (chunk_id, content_id, start, end) in enumerate(group):
        if content_id not in text_cache:
            text_cache[content_id] = get_source_text(content_id, "transcripts", storage_paths[content_id])
...
def fetch_round(batch_id: str) -> None:
    ...
    # so one shared round number for the whole batch mislabels most of
    # it. Look up each chunk's document and use 1 + however many distinct
    # rounds that SPECIFIC document has already been scored in, computed
    # fresh right before writing.
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def run_loop_with_threshold(content_ids: list[str], min_hits: int = 2, max_rounds: int = 3) -> None:
    ...
    Formalizes the manual ">=2 hits to continue" rule into something that
    can run several rounds unattended. Blocks on each batch via polling --
    intended for a bounded run, not indefinite use."""
    import time
